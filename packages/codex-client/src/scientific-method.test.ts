...
describe("hosted scientific-method compiler", () => {
  ...
  it("uses one bounded repair turn when immutable scientific lineage is invalid", async () => {
    const fakeServer = fileURLToPath(
      new URL("./test-fixtures/fake-app-server.mjs", import.meta.url),
    );
    const root = resolve(fileURLToPath(new URL("../../..", import.meta.url)));
    const work = join(
      root,
      `node_modules/.cache/counterlab-v6.1/test-work/codex-lineage-repair-${process.pid}`,
    );
    await mkdir(work, { recursive: true });
    const compiler = new AppServerCodexCompiler({
      command: process.execPath,
    ...
    expect(events).toContainEqual({
      type: "policy_repair",
      attempt: 1,
      findingCodes: ["IMMUTABLE_LINEAGE_MISMATCH"],
    });
    expect(events.at(-1)).toMatchObject({
      type: "file_change",
      status: "completed",

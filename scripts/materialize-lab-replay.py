...
def materialize(root: Path, image: str) -> None:
    ...
    temporary_root = create_repository_work_directory(root, "materialize-replay")
    ...
    for mutation in critical_mutations(outcome.result):
        ...
        detected = report["status"] == "REJECTED" and mutation[
            "expectedInvariant"
        ] in {failure["invariant"] for failure in report["failures"]}
        if not detected:
    ...
    verifier_report = _with_hash(
        {
            ...
            "verifiedInvariants": outcome.verification["verifiedInvariants"],

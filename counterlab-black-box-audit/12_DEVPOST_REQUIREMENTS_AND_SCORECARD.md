# 12 — DEVPOST REQUIREMENTS & STRICT SCORECARD

Audit date: 2026-07-19 · Verified deadline: **Tuesday 2026-07-21 17:00 PT (2026-07-22T00:00Z)** — triply documented on the live Devpost page (homepage ×2, FAQ, dates table) and corroborated by openai.com/build-week. **Remaining from audit date: ~3 days (T-72h from 2026-07-19 00:00 UTC; ~65h from 2026-07-19 00:00 PT).** Judging 2026-07-22 → ~Aug 9; winners ~Aug 12. Keep the app alive and as-depicted through **Aug 13** to be safe.

## Strict scorecard (post red-team challenge)

| State | Tech Impl | Design | Impact | Idea | Total |
|---|---|---|---|---|---|
| 1. Current public application | 17 | 18 | 17 | 21 | **73/100** (range 70–75) |
| 2. After confirmed P0+P1 corrections | 21 | 22 | 19 | 23 | **~85** (83–87, requires re-audit) |
| 3. Realistic prize-one ceiling | 23 | 22 | 22 | 23 | **~89** (88–90 without working live leg + outcome evidence) |

Two independent scorers (Agent 3 on /judge-only basis; Agent 16 blind pre-swarm) both arrived at **73/100**; the commander's synthesis concurs. Machine-readable: `12_DEVPOST_SCORECARD.json`.

### Technological Implementation — current 17/25 (range 15–19)
- Evidence supporting the score:
  - Complete 6-step sample loop in production on Cloudflare Workers
  - Hash-chained event log with actor attribution; all 20 chain links re-validated
  - Deterministic artifacts: repaired .ipynb SHA-256 recomputed = patchedArtifactHash
  - Fail-closed withholding on unknown IDs
  - Unauthenticated JSON API with consistent envelope; ~12.8s kernel compute
- Evidence preventing a higher score:
  - MB-004: Runtime Codex decorative in every reachable path (empty events, 360ms compile-verify, stored fixture)
  - MB-003: GPT-5.6 framing byte-identical template
  - MB-002: live leg dead-ends
  - MB-001: refusal paths brick with raw state errors
  - MB-009: dual result hashes, unexplained
  - MB-005: transparency drawer empty
- Strongest judge objection: 'The criterion is how thoroughly the project uses Codex — show me Codex doing anything in the product.'
- Highest-leverage action: Ship verifiable Codex proof: surface real codex payloads/plan-diff events in the drawer and link build-time Codex evidence (Session ID, README) from /judge
- Cannot verify without repository access:
  - build-time Codex usage (required by rules)
  - HMAC key separation
  - Docker sandbox enforcement
  - live-path GPT-5.6/Codex behaviour

### Design — current 18/25 (range 16–20)
- Evidence supporting the score:
  - Editorial /judge dossier with four-authority story
  - Experiment-Theater reveal and prediction-seal ritual
  - SAMPLE/LIVE/REPLAY badges + persistent replay banner
  - Honest limitation microcopy
  - Strong native semantics (fieldset/radios/selects/details)
- Evidence preventing a higher score:
  - MB-005: drawer '0 events' after completed session
  - MB-006: transfer ✓/× inversion
  - MB-011: 1.8:1 payoff text
  - MB-008: eternal replay spinner
  - MB-018: dead links
  - MB-030: split dark/light identity, unstaged reveal
  - MB-032/033: mobile risk + a11y barriers
- Strongest judge objection: 'The flagship transparency surface is empty and the one scored interaction contradicts itself.'
- Highest-leverage action: Fix MB-005 (data already server-side), MB-006 (copy/glyph swap), MB-011 (token change) — XS–S effort, immediate Design recovery
- Cannot verify without repository access:
  - nothing material — design is fully observable

### Potential Impact — current 17/25 (range 15–19)
- Evidence supporting the score:
  - Real, common learning problem (leakage-driven overconfidence)
  - Genuine falsification experience (sealed belief contradicted by evidence)
  - Honest scope disclosures
  - Fixed-code-scored transfer exists
- Evidence preventing a higher score:
  - Zero learner-outcome evidence anywhere
  - MB-013: system does the generative thinking; learner confirms
  - MB-007: pre-seal verdict leak defeats the prediction contract
  - MB-012: mastery overclaim from one 2-item gate
  - Single subject pack publicly reachable
- Strongest judge objection: 'Show me one learner whose behaviour changed.'
- Highest-leverage action: Run a minimum credible 6–12-person pre/post unassisted-transfer study before submission and publish the protocol + results on the project page
- Cannot verify without repository access:
  - none — outcome evidence is simply absent

### Quality of the Idea — current 21/25 (range 19–23)
- Evidence supporting the score:
  - Novel composition with no public equivalent found (independent scan + red-team concur)
  - Memorable ownable line: 'Ask like chat. Prove it like science.'
  - Bounded, self-aware claim scoping
  - Mechanizes established pedagogy (POE / Peer Instruction) with proof artifacts
- Evidence preventing a higher score:
  - MB-014: one-sentence novelty never stated; 'CI for understanding' reads dev-tools not Education
  - MB-037: no framing vs leakage linters / Socratic incumbents
  - MB-038: no visible refusal/inconclusive states on claims
- Strongest judge objection: 'Isn't this a linter plus a quiz?'
- Highest-leverage action: Add the one-sentence delta ('The first evidence-first learning tool: your notebook's reality grades your belief, and proof is a signed artifact') + prior-art framing to /judge hero and Devpost page
- Cannot verify without repository access:
  - originality vs the full Build Week field (gallery not public at audit date)

---

## Submission-compliance risk register (P0 = disqualification/incomplete-submission risk; all UNVERIFIED — submission materials are not public)

See MB-C01…MB-C13 in `06_ISSUE_REGISTER.md`. The six P0 risks: missing /feedback Codex Session ID; video without compliant voiceover; GPT-5.6 in-product evidence; Codex-in-build vs Codex-in-product conflation; repo/license/README accessibility; app availability through judging.

---

# APPENDIX — Agent 14's full rules research (verbatim quotes, sources, access dates)

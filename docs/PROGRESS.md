...
- **Containment implementation checkpoint:**
  ...
  assumption. The replacement untagged-OCI identity gate is locally green and
  awaits its source commit, rebuild, and complete refresh.
...
- **Post-audit-preservation working tree:** the concurrent screenshot set and
  ...
  current uncommitted OCI-identity gate, isolated Belief Break component,
...
| Formatting, whitespace, and secret scan | PASS | Prettier, Git whitespace checks, and the repository secret scan passed; the latest scan covered 798 files. |
...
| Exact OCI evidence retention | PASS locally; rebuild pending | Current vulnerability and VEX summaries now retain the validated OCI manifest digest instead of collapsing evidence back to the config digest. Current verification recomputes both reports from raw Grype evidence, requires one manifest identity across the three VEX scans, and rejects missing or cross-report manifest binding while preserving historical parser compatibility. Qualification now compares both reports directly to the build receipt before registry promotion. Both summarizer CLIs reject escaped/symlinked paths, unknown or duplicate flags, and output replacement. Focused root Vitest passes 4 files/31 tests and the focused Worker release test passes 1 file/15 tests; repository TypeScript, shell syntax, Prettier, and Git whitespace checks pass. |
...
| Verified Belief Break component | PASS in isolation; integration/browser pending | A new fixed-evidence `VerifiedBeliefBreakTheater` was created only under the learner component boundary. It verifies the exact checked-in sample bytes, result lineage, Boundary fixture, and controlled runs before exposing values; keeps a persistent sample label; provides non-color mechanism and exact-value table; and makes no model, runner, or network call. Focused Vitest passed 2 files/12 tests, web TypeScript and Prettier passed. `App.tsx` and Judge integration remain lead-owned and not yet changed. |
| Learner-pilot evidence hardening | PASS in isolation; integration pending; outcomes remain `NO_DATA` | Pilot v2 schemas and analysis now fail closed on assignment/order or consent-reference mismatch and add first-unassisted transfer, bounded confusion/abandonment, and closed reactions without PII, free text, consent content, or participant rows. Focused Vitest passed 1 file/12 tests; isolated TypeScript and Prettier passed. The root analyzer and study documents still require lead integration. No learner-impact result is claimed. |
...
| Broad product/runtime verification | IN PROGRESS | Web Vitest 59 files/375 tests; Python kernel and runner 234/234; scientific verifier/reachability 10/10; leakage mutations 13/13; imbalance mutations 19/19; root, web, Worker, and hosted-runner TypeScript; production build; scientific import audit; whitespace; formatting; and the 798-file secret scan passed locally. Pre-refresh root Vitest passed 42/44 files and 499/502 tests; the three stale-evidence failures remain queued for exact-image refresh. |

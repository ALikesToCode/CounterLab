...
| Contained-runtime FIFO isolation | PASS implementation/toolchain checks; real task pending | Exact nerdctl 2.3.1 source confirms that its client I/O helper hardcodes `/run/containerd/fifo` with no flag or environment override. Exact containerd 2.3.1 source and pinned CLI help confirm that `ctr tasks start --fifo-dir` is the supported override. The contained runtime now has nerdctl create the already validated OCI spec, starts only that named task through pinned `ctr` with `.rt/<session>/run/client-fifo`, and performs the same ephemeral cleanup. The `ctr` 2.3.1 binary is hash-bound in the toolchain lock; the FIFO directory and new helper are attested. Focused Vitest passes 1 file/16 tests, including fail-closed cleanup; Prettier, Node syntax, shell syntax, and existing-install toolchain verification pass. Unprivileged launch `rt-v61-0719r` was stopped by the managed sandbox's `newuidmap` capability boundary. Privileged fresh launch `rt-v61-0719s` reached attestation and failed closed because the new path/helper were absent from two exact-key allowlists; no task ran. The allowlists and a launcher/verifier parity regression are now repaired. No real container pass is claimed until another fresh runtime executes the profile. |

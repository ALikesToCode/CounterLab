...
| Contained-runtime FIFO isolation | PASS implementation/toolchain checks; real task pending | Exact nerdctl 2.3.1 source confirms that its client I/O helper hardcodes `/run/containerd/fifo` with no flag or environment override. Exact containerd 2.3.1 source and pinned CLI help confirm that `ctr tasks start --fifo-dir` is the supported override. The contained runtime now has nerdctl create the already validated OCI spec, starts only that generated container ID through pinned `ctr` with `.rt/<session>/run/client-fifo`, and performs ephemeral cleanup through the independently validated friendly name. The `ctr` 2.3.1 binary is hash-bound in the toolchain lock; the FIFO directory and new helper are attested. Focused Vitest passes 1 file/17 tests, including invalid-ID and fail-closed cleanup paths; Prettier, Node syntax, shell syntax, and existing-install toolchain verification pass. Unprivileged launch `rt-v61-0719r` was stopped by the managed sandbox's `newuidmap` capability boundary. Privileged fresh launch `rt-v61-0719s` reached attestation and failed closed because the new path/helper were absent from two exact-key allowlists; no task ran. Fresh `rt-v61-0719t` then attested `VERIFIED`, imported the retained `74d60a7` OCI archive in 161.5 seconds, and proved the FIFO directory itself writable and self-cleaning. Its first task start failed closed because nerdctl's friendly name is not the generated containerd ID; cleanup ran and no FIFO remained. The helper now validates and hands off only nerdctl's 64-hex generated ID. No real container pass is claimed until a fresh runtime executes the repaired helper. |

...
| Contained-runtime FIFO and shim isolation | PASS local implementation/preflight; real task pending | Exact nerdctl 2.3.1 source confirms that its client I/O helper hardcodes `/run/containerd/fifo` with no flag or environment override. Exact containerd 2.3.1 source and pinned CLI help confirm that `ctr tasks start --fifo-dir` is the supported client override. The contained runtime now has nerdctl create the validated OCI spec, starts only that generated container ID through pinned `ctr` with `.rt/<session>/run/client-fifo`, and performs ephemeral cleanup through the independently validated friendly name. The `ctr` 2.3.1 binary is hash-bound in the toolchain lock; the FIFO directory and helper are attested. Earlier launches `r` through `u` isolated managed-sandbox, attestation, generated-ID, and v2 shim-default failures without releasing a result. Official containerd 2.3.1 Linux source identifies the supported `plugins.'io.containerd.shim.v1.manager'.socket_dir` setting and its 42-character limit. The verified repository root is exactly 42 characters, so the repair configures that root directly, disables unused CRI/NRI plugins, and preflights a full 107-byte transient socket plus automatic cleanup before containerd launch; it does not use `/proc`, `/run`, `/tmp`, or an out-of-repository alias. The managed command sandbox blocks Unix socket binds with `EPERM`, so the pure contract suite remains sandbox-safe while the production launcher owns the real preflight. Outside that sandbox, the exact preflight passed and left no socket. Focused Vitest passes 2 files/22 tests; repository and web TypeScript, Node/Bash syntax, Prettier, and Git whitespace checks pass. No real container pass is claimed until a fresh rootless runtime runs the sentinel. |

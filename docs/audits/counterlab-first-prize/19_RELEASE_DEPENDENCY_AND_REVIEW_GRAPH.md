...
flowchart TD
  ...
  C["Clean source freeze S<br/>one committed physical tree<br/>BLOCKED by preserved FUSE entry"]
...
| Clean source `S`          | Final intended source and no tracked or untracked drift                                                                    | `git status` empty and exact source commit recorded                                                                                                                               | **Blocked** by preserved FUSE entry                                                                                                               | Do not hide, delete, or sweep the entry; use its proven owner's natural exit or a separately contained clean checkout        |
...
2. The current constitution prohibits the sentinel's necessary `/sys/fs/cgroup`
   ...
   path is established.
3. The preserved FUSE entry prevents the required clean physical source tree.
4. No current-source image, scientific-evidence refresh, evidence commit,
   qualified receipt, or release-check receipt exists.
5. Contained Wrangler authentication and the intended Cloudflare account have
